...
max_price = st.sidebar.slider('Maximum Price', books_df['Price'].max(),)
...
if selected_year != "All":
    filtered_books_df = filtered_books_df[filtered_books_df['Year'] == selected_author]
...
filtered_books_df = filtered_books_df[
    (filtered_books_df['User Rating'] >= min_rating) & (filtered_books_df['Price'] <= min_rating)
]
...
total_books = filtereed_books_df.shape[0]
unique_titles = filtered_books_df['Name'].unique()
...
with col2:
    ...
    top_titles = filtered_books_df['Authors'].value_counts().head(10)
    st.bar_chart( top_titles)

st.subheader("Genre Distriubution")
